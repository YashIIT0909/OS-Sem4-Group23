...
## ⚔️ Step 3: Task Distribution

There are 4 sections. **Dinesh is taking Section 1.** Please reply in our group chat to claim Section 2, 3, or 4.

### 👤 Section 1: The Foundation (Assigned to: Dinesh)
...
* **Call 4: `sys_ps()`** - Iterates through the kernel's process table and prints all active processes (like the `ps` command).
...
* **The User App (`testapp.c`):** Write the user-space C program that calls and tests EVERY system call made by the team. You will be responsible for taking the execution screenshots for the final report.
